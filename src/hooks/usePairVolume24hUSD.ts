...
import { CurrencyAmount, Pair, USD } from 'defimist-sdk'
...
interface QueryResult {
  pairDayDatas: { dmtlyVolumeUSD: string }[]
}
  ...
  return useMemo(() => {
    ...
    return {
      ...
      volume24hUSD: CurrencyAmount.usd(
        parseUnits(new Decimal(data.pairDayDatas[0].dmtlyVolumeUSD).toFixed(USD.decimals), USD.decimals).toString()
      )
    }
  }, [data, error, loading])
}
