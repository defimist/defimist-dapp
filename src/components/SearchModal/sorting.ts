import { Pair, Token, TokenAmount } from 'defimist-sdk'
import { CurrencyAmount } from 'defimist-sdk/dist/entities/fractions/currencyAmount'
import { useCallback, useMemo } from 'react'
import { useActiveWeb3React } from '../../hooks'
import { toDefimistLiquidityToken, useTrackedTokenPairs } from '../../state/user/hooks'
...
export function usePairsComparator(inverted: boolean): (pairA: Pair, pairB: Pair) => number {
  ...
  const balances = useTokenBalances(
    account || undefined,
    trackedTokenPairs.map(rawPair => toDefimistLiquidityToken(rawPair))
  )
