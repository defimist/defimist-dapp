import { TokenAmount, Pair, Currency, RoutablePlatform } from 'defimist-sdk'
import { useMemo } from 'react'
import { abi as IDefimistPairABI } from 'defimist-core/build/IDefimistPair.json'
...
const PAIR_INTERFACE = new Interface(IDefimistPairABI)
...
export function usePairs(
  ...
  return useMemo(() => {
    return results.map((result, i) => {
      ...
      if (!swapFee && platform === RoutablePlatform.DEFIMIST) return [PairState.LOADING, null]
      return [
        PairState.EXISTS,
        new Pair(
